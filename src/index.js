...
const COURSES = [
  {
    COURSE_SLUG: "reactivity-solidjs",
    PREFERRED_QUALITY: "index_1080_Q10_7mbps",
  },
  {
    COURSE_SLUG: "api-design-nodejs-v4",
    PREFERRED_QUALITY: "index_1080_Q10_7mbps",
  },
  {
    COURSE_SLUG: "unity",
...
if (!process.env.FEM_AUTH_MOD) {
  console.log("Please set FEM_AUTH_MOD in .env file")
  process.exit()
}

for (const { COURSE_SLUG, PREFERRED_QUALITY } of COURSES) {
  const EXTENSION = "mp4"
  const INCLUDE_CAPTION = true
  const DOWNLOAD_DIR = safeJoin(os.homedir(), "Downloads")

  const TOKEN = process.env.FEM_AUTH_MOD
  ...
  for (const [lesson, episodes] of Object.entries(lessons)) {
    ...
    for (const episode of episodes) {
      ...
      await Promise.all([
        ...
        fs.writeFile(
          playlistPath,
          m3u8.replace(/URI="(.+)"/g, 'URI="key.bin"')
        ),
      ])
